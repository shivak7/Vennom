...
class VNNM_Integrate 
{
	double h_coeffs[3];
	public:
	long int num_steps;
	VNNM_Integrate(){num_steps = 0;}
	void EulerIntegrate(std::vector<VNNM_System *>& ,double &t, double dt);
	void RK4init(int);
	void Update(std::vector<VNNM_System *>&);
	void RK4Integrate(std::vector<VNNM_System *>& ,double &t, double dt);
	void RKCK(std::vector<VNNM_System *>& ALL,double t, double dt);
	void RKQS(std::vector<VNNM_System *>& ALL,double &t, double dt, double& dt_next, double eps);
	void RKAdaptive(std::vector<VNNM_System *>& ALL,double &t, double dt, double Tstart, double Tend, double dt_min);
	
	//template <typename T> void rk4_step( T * obj, double *y,double *ydiff,double *y_old,int nODE, double step, double x0);

};


/*
template <typename T>  void VNNM_Integrate :: rk4_step( T * obj, double *y,double *ydiff,double *y_old,int nODE, 
