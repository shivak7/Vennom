...
extern "C" {
	...
	{
		return 0.5*(1+ tanh((V - m0)/s0));
	}
	
	double x_tau(double V, double m1, double s1)
	{
		
		return 1.0/(cosh((V-m1)/(2*s1)));
	...
	int diff (int Nstates, double t, double dt, const std::vector<double> & Params, const std::vector<double> & curr_state, std::vector<double> & diff_state, std::vector<int> & integrate_flags, int Order, VNNM_System * Parent)
	{
		...
		for(int i=0;i<Nstates;i++)
		{
			...
			{
				diff_state[i] = Params[2+Nstates+i]*(x_inf(V,Params[p_step+0],Params[p_step+1]) - curr_state[i])/x_tau(V,Params[p_step+2],Params[p_step+3]);
			}
			p_step+=4;
		}
