...
//------------------------------------------------------------------------------------------------------------
	...
	double x_tau(double V, double k, double t0, double m0, double s0, double t1, double m1, double s1)
	{
	
		double a = t0*exp(-(V - m0 )/s0);
		double b = t1/(1 + exp(-(V-m1)/s1));
	...
	int diff (int Nstates, double t, double dt, const std::vector<double> & Params, const std::vector<double> & curr_state, std::vector<double> & diff_state, std::vector<int> & integrate_flags, int Order, VNNM_System * Parent)
	{
		...
		for(int i=0;i<Nstates;i++)
		{
			...
			{
				diff_state[i] = Params[2+Nstates+i]*(x_inf(V,Params[p_step+0],Params[p_step+1]) - curr_state[i])/x_tau(V,Params[p_step+2],Params[p_step+3],Params[p_step+4],Params[p_step+5], Params[p_step+6], Params[p_step+7], Params[p_step+8]);
			}
			p_step+=9;
		}
